...
class Bot:
    ...
    def get_move(self, board):
        ...
        if not board.check_color == self.color:
                ...
                for field in row:
                    if not field.empty and field.figure.color == self.color:
                        for m in field.possible_moves(board.fields):
                            self.make_move_simulation(field, m)
            ...
            for moves in board.available_moves_on_check:
                fie, mov = moves
                self.make_move_simulation(fie, mov)
                if self.evaluation < min:
                    min = self.evaluation
    ...
    def set_evaluation(self, fields):
        print(id(fields[7][1]), fields[7][1].symbol)
        print(id(self.orginal_f[7][1]), self.orginal_f[7][1].symbol)
        self.draw_fields(fields)
        evaluation = 0
        for i, row in enumerate(fields):
            for j, field in enumerate(row):
                if field.pos_x == 7 and field.pos_y == 1:
                    pass
                    # print(evalll_fig['evalu'][5][5])
                    # print(evalll_fig)
                    # print(field.symbol)
                if not field.empty:
                    symbol = field.symbol
