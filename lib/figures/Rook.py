...
class Rook:
    ...
    def get_possible_moves(self, board, pos_x, pos_y):
        ...
        for i in range(pos_x - 1, -1, -1):
            if board.fields[i][pos_y].empty:
                moves.append((i, pos_y))
            elif board.fields[i][pos_y].figure.color != self.color:
                moves.append((i, pos_y))
                break
            else:
                break
        ...
        for i in range(pos_y - 1, -1, -1):
            if board.fields[pos_x][i].empty:
                moves.append((pos_x, i))
            elif board.fields[pos_x][i].figure.color != self.color:
                moves.append((pos_x, i))
                break
            else:
                break
